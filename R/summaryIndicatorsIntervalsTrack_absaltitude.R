...
#'
#' \code{summaryIndicatorsIntervalsTrack_absaltitude} computes the sum of
#' the absolute height differences (i.e. all covered height meters)
#' a household covered during a fixed ten-day interval. Absolute height
#' distances are
...
#' @examples #
#' @export
summaryIndicatorsIntervalsTrack_absaltitude <- function(currenttrack,
                                                        normalise = FALSE) {
  ...
  # compute absolute altitudinal differences between all adjacent data values
  connections$altitude <- abs(currenttrack$altitude[-1] - currenttrack$altitude[-length(currenttrack$altitude)])
