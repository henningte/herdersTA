...
#'
#' \code{gapDuration} computes the duration of gaps [s] for all
#' \code{\link[trajectories:Track-class]{Track}} objects within a given
#' \code{\link[trajectories:Track-class]{Tracks}} object and within a given time
#' period, using the function \code{\link{gapDuration}}.
#'
...
#' \code{\link[trajectories:Track-class]{Tracks}} object [s].
#' @seealso \code{\link{findGaps}}, \code{\link{countGaps}},
#' \code{\link{gapProportion}}, \code{\link{totalGapProportion}}.
#' @examples #
#' @export
totalGapDuration <- function(currenttracks,
                             tstart = min(currenttracks@tracksData$tmin),
                             tend = max(currenttracks@tracksData$tmax),
                             threshold = 18000,
                             tolerance = 2) {

  # compute the duration of gaps within the Tracks object
  sum(unlist(lapply(currenttracks@tracks,
                    gapDuration,
                    tstart = tstart,
                    tend = tend,
                    threshold = threshold,
