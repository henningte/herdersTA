...
#' position and time of a \code{\link[trajectories]{Track}} object with
#' daily resolution.
#'
...
extractRasterTrack <- function(currenttrack,
  ...
  extractedvalues <- do.call(cbind, raster::extract(x = raster, y = locationsdata, method = "simple", buffer = buffer, na.rm = TRUE, layer = 1, nl = nlayers(raster)))
